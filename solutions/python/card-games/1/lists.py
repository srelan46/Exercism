...
def approx_average_is_average(hand):
    ...
    """
    length = len(hand)
    average_first_last = (hand[0]+hand[length-1])/2
    middle = hand[length//2]
    average = card_average(hand)
    return average == average_first_last or average == middle
...
def average_even_is_average_odd(hand):
    ...
    for index in range(len(hand)):
        if index % 2 == 0:
            even_count += 1
            even_total += hand[index]
        else:
            odd_count += 1
            odd_total += hand[index]
...
def maybe_double_last(hand):
    ...
    length = len(hand)
    if hand[length-1] == 11:
        result = hand[:length-1]+ [hand[length-1]*2]
    else:
        result = hand
    return result